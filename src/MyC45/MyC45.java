...
public class MyC45 {
	public static void main(String[] args) throws IOException{
		...
		while(scan.hasNextLine()){
			...
			//inset data into attributes
			for(int x = 0; x < numAttributes; x++){
				data = new Val(lineData[x], lineData[classIndex]);
				attributes[x].inserVal(data);
			}
		}
		...
		Val inV = new Val("30","yes"); age.inserVal(inV);
		inV = new Val("30","yes"); age.inserVal(inV);
		inV = new Val("30","no"); age.inserVal(inV);
		inV = new Val("30","no"); age.inserVal(inV);
		inV = new Val("30","no"); age.inserVal(inV);
		inV = new Val("35","yes"); age.inserVal(inV);
		inV = new Val("35","yes"); age.inserVal(inV);
		inV = new Val("35","yes"); age.inserVal(inV);
		inV = new Val("35","yes"); age.inserVal(inV);
